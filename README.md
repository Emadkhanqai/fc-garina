...
- **Opening toss** — dramatic draw (fast → slow → fast → slow → settle) for Match 1; teams stay hidden as `? ? ?` until the roulette starts. Once it settles you choose RE-ROLL (run it again) or ACCEPT MATCH 1 to lock it in — nothing is generated until you accept. Skipped automatically for 2-team tournaments, where the single possible pairing locks immediately.
