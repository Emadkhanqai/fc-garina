...
- Under each nameplate: an **FC26 in-game team input** (which real-world national/club team that player picked, e.g. FRANCE). Dashed bottom border in team color, transparent-dark bg, centered uppercase Rajdhani 700 `13px` in team color, placeholder `FC26 TEAM?`, max 16 chars. Its parent gets `position:relative` for a custom suggestion dropdown — **not** the native `<datalist>`, which iOS Safari effectively ignores on text inputs. On focus (and while typing, filtered to picks containing the substring, self-match excluded, capped at 6) a `150px`-wide panel drops below the field: `#0d1118`, `1px` border in the team's color, radius `10px`, each pick a full-width unstyled button (Rajdhani 700 `13px`, divider `1px rgba(255,255,255,.06)`, tap state `rgba(255,255,255,.08)`). Closing on blur is delayed `180ms` so a tap on a suggestion registers before the panel unmounts; picking one commits the value immediately and closes.
