...
- **Print** renders the real UI, in colour (`print-color-adjust: exact`, canvas stays `#06080c`, `@page` margin `10mm`), but only the record sections: **league table, top scorers and fixtures**. Everything else carries `[data-noprint]` — hero, Meet the Teams, opening toss, the live match card and its Record Goal band, Next Match, Grand Stage, the header ⋯, fixture ▶ buttons, every overlay backdrop/sheet and the toast — leaving the compact brand row as the report's title. `[data-printblock]` sets `break-inside: avoid` on each fixture row and panel. The text summary (`[data-printonly]`) follows on a new page, restyled dark to match.
